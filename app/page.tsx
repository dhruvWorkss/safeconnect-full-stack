...
export default function Home() {
  ...
  const visible = useMemo(() => cases.filter((item) => !resolved.includes(item.id) && (filter === "All cases" || item.severity === filter) && `${item.id} ${item.user} ${item.reason}`.toLowerCase().includes(query.toLowerCase())), [query, filter, resolved]);
  const act = (message: string, resolve = false) => { if (resolve) setResolved((items) => [...items, active.id]); setToast(message); setTimeout(() => setToast(""), 2600); };
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-card"><span className="pulse-dot"/><div><strong>Systems healthy</strong><small>All services operational</small></div></div>
    ...
    <section className="workspace">
      ...
      <div className="content-grid">
        ...
        <aside className="case-detail panel"><div className="case-top"><div><span className={`severity ${active.severity.toLowerCase()}`}>{active.severity} risk</span><h2>{active.id}</h2></div><button aria-label="More options">•••</button></div>
          ...
          <div className="action-box"><button className="resolve" onClick={() => act(`${active.id} resolved and archived`, true)}>Resolve case</button><button className="escalate" onClick={() => act(`${active.id} escalated to senior review`)}>Escalate</button></div><button className="history" onClick={() => act("Audit history loaded")}>View full case history <span>→</span></button>
        </aside>
      </div>
