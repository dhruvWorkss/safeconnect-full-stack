...
from moderation.authentication import WorkspaceTokenView
...
urlpatterns=[path("admin/",admin.site.urls),path("api/health/",health),path("api/auth/register/",RegisterView.as_view()),path("api/auth/token/",WorkspaceTokenView.as_view()),path("api/auth/token/refresh/",TokenRefreshView.as_view()),path("api/dashboard/stats/",dashboard_stats),path("api/",include(router.urls))]
